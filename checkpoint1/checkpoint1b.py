...
def fix_caffeine(df):
    ...
    df = df[df['Caffeine (mg)'] != 'varies']
    df['Caffeine (mg)'] = [float(x) for x in df['Caffeine (mg)'].values]
    return df

def standardize_names(df):
    df.columns = ['beverage category', 'beverage', 'beverage_prep', 'calories',
       'total fat', 'trans fat', 'saturated fat', ' sodium',
       'total carbohydrates', 'cholesterol', 'dietary fibre',
       'sugars', 'protein', 'vitamin a', 'vitamin c',
       'calcium', 'iron', 'caffeine']
    return df

def fix_strings(df, col):
    df[col] = df[col].replace(r'[^a-zA-Z ]\s?',r'',regex=True)
